...
class SentimentAnalysisExplainer(mlflow.pyfunc.PythonModel):
    ...
    def sentiment_classifier_explanation(self, row):
        ...
        # MLflow pyfunc model input and output signature do not support complex object without serialization.
        # Make model output compatible with MLFlof by serialized SHAP values using pickle.
        return [pickle.dumps(shap_values)]

    def predict(self, context, model_input):
        model_input[['shap_values']] = model_input.apply(self.sentiment_classifier_explanation, axis=1,
                                                         result_type='expand')
